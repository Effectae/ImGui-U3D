...
unsigned int getPtr32(unsigned int addr)
{
	...
	unsigned int var = 0;
	vm_readv(addr & 0xFFFFFFFFFF, &var, 4);
...
string getmiddle(string text, string head, string tail)
{

	int mhead = text.find(head, 0);
	int mtail = text.find(tail, mhead);
	int headend = mhead + head.length();
	int langth = mtail - headend;
	return text.substr(headend, langth);
};
